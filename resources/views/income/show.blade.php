...
@section('content')
    ...
    <div class="px-6 py-4">
        ...
        <hr class="pb-2 mt-2">
        <p class="mb-4"><strong>Total Income Amount: </strong> {{ $monthlyPlan->income_amount }}</p>
        <div class="flex mb-4">
            <p class="mr-6"><strong>Targeted Expense Amount: </strong> {{ $monthlyPlan->targeted_expense_amount }}</p>
            <p><strong>Total Expense Amount: </strong> {{ $expenses->where('expense_type', \Azim1993\ExpensePlanner\Data\ExpenseTypeEnum::DAILY_EXPENSE->value)->first()?->expenses }}</p>
        </div>
        <div class="flex mb-4">
            <p class="mr-6"><strong>Targeted Investment Amount: </strong> {{ $monthlyPlan->targeted_investment_amount }}</p>
            <p><strong>Total Investment Amount: </strong> {{ $expenses->where('expense_type', \Azim1993\ExpensePlanner\Data\ExpenseTypeEnum::INVEST->value)->first()?->expenses }}</p>
        </div>
        <p class="mb-4"><strong>Total Expense Amount: </strong> {{ $expenses->sum('expenses') }}</p>
