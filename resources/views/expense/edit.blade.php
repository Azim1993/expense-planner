...
@section('content')
    <section class="flex justify-between w-full items-center border-b px-6 pb-4">
        <h3 class="text-xl font-bold">Update Expense</h3>
        ...
        <form action="{{ route('expenses.update', $expense) }}" method="post">
            ...
            <div class="grid gap-x-8 gap-y-4 grid-cols-3">
                    ...
                    <div class="relative">
                        <select name="monthly_plan_id" id="monthly_plan_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline leading-tight focus:outline-none">
                            ...
                            @foreach($monthlyPlans as $monthlyPlan)
                                <option
                                    @if($expense->monthlyPlan->id == $monthlyPlan->id) selected @endif
                                    value="{{ $monthlyPlan->id }}">{{ $monthlyPlan->plan_month . ' - '. $monthlyPlan->plan_year }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select name="expense_type" id="expense_type" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline leading-tight focus:outline-none">
                            ...
                            @foreach($types as $type)
                                <option
                                    @if($expense->expense_type == $type) selected @endif
                ...
                <div class="col-span-2">
                    ...
                    <input name="title" value="{{old('title', $expense->title)}}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="title" type="text" placeholder="Title">
                </div>
                <div></div>
                <div class="col-span-2">
                    ...
                    <input name="expense_amount" value="{{old('expense_amount', $expense->expense_amount)}}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="expense_amount" type="number" placeholder="Expense Amount">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="description" placeholder="Description">{{ old('description', $expense->description) }}</textarea>
                </div>
            </div>
            <div></div>
            <div class="mt-6">
